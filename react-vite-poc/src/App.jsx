...
function App() {
  ...
  return (
    <div className='container m-3 p-3'>
      <h1 className='m-3 p-3 text-center'>People List</h1>
      <>
      {loading ? (
      ...
      <Accordion className='w-100'>
        {people.map((person) => (
        <Accordion.Item className='w-100' eventKey={person.id}>
          <Accordion.Header className='w-100'><img src="/profile-250x250.jpg" style={{borderRadius: '50%'}} className='m-1 p-1' height='50' width='50'/><strong>{person.name}</strong> — {person.motto} - Distance: {person.distance} miles -  Likes Dogs: {person.dogs}</Accordion.Header>
          <Accordion.Body>
            <div>
